...
void AnalysisStop::Loop(TString analysis, TString filename, float luminosity, float StopRefMass, float NeutralinoRefMass)
{
  ...
  for (Long64_t jentry=0; jentry<_nentries;jentry++) {
	...
	if (_htgen>70.) continue;
      ...
      if (!_systematic.Contains("fake") && _ntightlepton < 3) continue; // at least 3 tigth leptons
...
//      if (_systematic.Contains("SS") && Lepton1.flavour * Lepton2.flavour < 0) continue;
      ...
      if (AnalysisLeptons[0].v.Pt() < 40.) continue;
...
>>>>>>> upstream/master
    ...
    if (!_isminitree) 
    {
     ...
     // if (passZmass) std::cout << "passZmass" << std::endl; 
     // if (passTTZ) std::cout << "passTTZ" << std::endl; 
     // if (passTTZexerc) std::cout << "passTTZexerc" << std::endl;
 
      FillLevelHistograms(Stop_00_TTZexerc, passTTZexerc); 
      FillLevelHistograms(Stop_00_TTZ, passTTZ);
...
// ****
	...
	if (_SaveHistograms==0) DefineHistograms(i, j, k, suffix);
        ...
        h_counterLum_TTZexe   [i][j][k] = new TH1F("h_counterLum_TTZexe"    + suffix, "",    3, 0,    3);
        h_counterLum_TTZ      [i][j][k] = new TH1F("h_counterLum_TTZ"       + suffix, "",    3, 0,    3);
        h_m2l_TTZexe          [i][j][k] = new TH1F("h_m2l_TTZexe"           + suffix, "", 2000, 0, 2000);
        h_m2l_TTZ             [i][j][k] = new TH1F("h_m2l_TTZ"              + suffix, "", 2000, 0, 2000);
        h_metPfType1_TTZexe   [i][j][k] = new TH1F("h_metPfType1_TTZexe"    + suffix, "", 2000, 0, 2000);                
        h_metPfType1_TTZ      [i][j][k] = new TH1F("h_metPfType1_TTZ"       + suffix, "", 2000, 0, 2000);                
	h_MT2ll_TTZexe        [i][j][k] = new TH1F("h_MT2ll_TTZexe"         + suffix, "",    7,    0,  140);
	h_MT2ll_TTZ           [i][j][k] = new TH1F("h_MT2ll_TTZ"            + suffix, "",    7,    0,  140);
...
void AnalysisStop::FillAnalysisHistograms(int ichannel,
					  int icut,
					  int ijet)
{ //std::cout << "FillAnalysisHistograms   "  << "channel = " << schannel[ichannel] << "  icut = " << scut[icut] << "  ijet  = " << ijet << std::endl; 
  // ****
  if (ichannel != lll) FillAnalysisHistograms(lll, icut, ijet);
  // ****
  
  h_counterLum_TTZexe    [ichannel][icut][ijet]->Fill(1, _event_weigth_TTZ_susy);

  h_counterLum_TTZ      [ichannel][icut][ijet] ->Fill(1,        _event_weight);
  h_m2l_TTZexe          [ichannel][icut][ijet] ->Fill(_m2l,     _event_weigth_TTZ_susy);
  h_m2l_TTZ             [ichannel][icut][ijet] ->Fill(_m2l,     _event_weight      );
  h_metPfType1_TTZexe   [ichannel][icut][ijet] ->Fill(MET.Et(), _event_weigth_TTZ_susy);
  h_metPfType1_TTZ      [ichannel][icut][ijet] ->Fill(MET.Et(), _event_weight);
  h_MT2ll_TTZexe        [ichannel][icut][ijet] ->Fill(_MT2ll,   _event_weigth_TTZ_susy );
  h_MT2ll_TTZ           [ichannel][icut][ijet] ->Fill(_MT2ll,   _event_weight );
