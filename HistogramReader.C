				      ...
				      TString level)
{
  ...
  TH1D* test_hist2 = (TH1D*)file->Get(level + "/h_counterLumTTZ_susy_evolution");

//  if (test_hist)  { std::cout << "test_hist " << std::endl; return;} 
//  if (test_hist2) { std::cout << "test_hist2 " << std::endl;} 
  
  if (test_hist && test_hist2){std::cout << " ciao" << std::endl;  return;}
  ...
  Int_t firstchannel = (level.Contains("Stop/")) ? eee : ee;
  Int_t lastchannel  = (level.Contains("Stop/")) ? lll : ll;
  ...
  TH1D* hist2 = new TH1D("h_counterLumTTZ_susy_evolution", "", nbins, -0.5, nbins-0.5);

  for (Int_t i=firstchannel, bin=0; i<=lastchannel; i++)
    {
      TH1D* dummy  = (TH1D*)file->Get(level + "/h_counterLum_" + schannel[i]);
      TH1D* dummy2 = (TH1D*)file->Get(level + "/h_counterLumTTZ_susy_" + schannel[i]);
     ...
     //----------------------------------------------- 
      hist2->SetBinContent(bin, Yield(dummy2));
    //  std::cout << " SetBinContent:     " <<  " bin = " << bin << " Yield = " << Yield(dummy2) << std::endl;  
      hist2->SetBinError  (bin, Error(dummy2));

      hist2->GetXaxis()->SetBinLabel(bin, lchannel[i]);
    }
