...
void Cut_JetPt_Study() {
  ...
  for (int R= 0; R < 6; R++){
    for (int dt = 0; dt<2; dt++) {
     ...
     DISCR[dt+dt*R][17]  = new TH1F("MT2ll"         + HistoName, "", 120, 20., 150.);
 ...
 for (int dt = 0; dt<2; dt++) {
  ...
  CC->Print("./CutsPt_Study/20csvv2m/" + svar[vv] + ".png");
  CC->Print("./CutsPt_Study/20csvv2m/" + svar[vv] + ".pdf");
 } 
}
