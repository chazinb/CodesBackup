...
const int nsignal = 1; const TString tssignal [nsignal] = { "T2tt_Sm350_Xm225"}; const TString massP [nsignal] = { "(350,225)"};
...
float ch_totB [nregion][nchannel]; 
...
void Table_txt () {
  ...
  //create the txt file
  std::ofstream inFile("Yields/20Dic_bVETO_Yields.txt",std::ios::out);  
  
  inFile << Form ("\n  \n"); 
  inFile << Form ("             TAG SELECTION =  BVETO                \n");
  inFile << Form ("\n  \n"); 
...
void PiePlot ( int SignalSample, int iregion){
  ...
  int colors[nbck +1] = {2,5,41,3,7,8,28};
   ...
   gSystem->mkdir("iplots/Yield/", kTRUE);
  ...
  for ( int ch = 0; ch < nchannel; ch ++)
  {
   ...
   for (int i = 0; i < nsample; i++)
   {
    ...
    const char* leglabel[7]  = { massP [SignalSample], "TTbar", "ST", "DYJets", "WW", "TTV", "VZ"};
   ...
   pie->Draw("3d <"); 
   pieleg -> Draw();
   cpie -> Print("iplots/Yield/bVETO_" +   tssignal [SignalSample] + "_" + sregion[r] + "_" + schannel[ch]+ ".png");
  } 
